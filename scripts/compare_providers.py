    ...
    if output_format == "json":
        ...
        if "openai" in df["Provider"].values and "claude" in df["Provider"].values:
            ...
            for metric in [
                ...
                if (metric, "openai") in pivot_df.columns and (
                    metric,
                    "claude",
                ) in pivot_df.columns:
                    diff_df[f"{metric} Diff"] = (
                        pivot_df[metric, "openai"] - pivot_df[metric, "claude"]
                    )
                    diff_df[f"{metric} % Diff"] = (
                        (pivot_df[metric, "openai"] / pivot_df[metric, "claude"]) - 1
                    ) * 100
...
def main():
    ...
    for story_file in story_files:
        ...
        # Save OpenAI result
        with open(
            os.path.join(args.output_dir, f"{story_name}_openai.json"), "w", encoding="utf-8"
        ) as f:
            json.dump(openai_result, f, indent=2)

        # Save Claude result
        with open(
            os.path.join(args.output_dir, f"{story_name}_claude.json"), "w", encoding="utf-8"
        ) as f:
