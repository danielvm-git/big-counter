...
class OpenAIProvider(LLMProvider):
    ...
    def __init__(
        self, logger: logging.Logger, model_name: str = "gpt-4o-2024-05-13", temperature: float = 0
    ):
        """
...
class ClaudeProvider(LLMProvider):
    ...
    def __init__(
        self,
        logger: logging.Logger,
        model_name: str = "claude-3-sonnet-20240229-v1:0",
        temperature: float = 0,
    ):
        """
    ...
    def get_model(self) -> BaseLanguageModel:
        ...
        return ChatAnthropic(
            model_name=self.model_name, temperature=self.temperature, timeout=None, stop=None
        )
...
class FlowChatModel(BaseChatModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values["base_url"] = (
            values.get("base_url") or "https://flow.ciandt.com/ai-orchestration-api/v1/openai"
        )
...
class FlowProvider(LLMProvider):
    ...
    def get_model(self) -> BaseLanguageModel:
        ...
        Returns:
            The Flow model
        """
        assert self.base_url is not None, "FLOW_BASE_URL must be set"
...
class FlowBedrockProvider(LLMProvider):
    ...
    def __init__(
        ...
        self.anthropic_version = os.environ.get(
            "FLOW_BEDROCK_ANTHROPIC_VERSION", "bedrock-2023-05-31"
        )
...
def get_provider(provider_name: str, logger: logging.Logger) -> LLMProvider:
    ...
    if provider_name == "deepseek":
        ...
        return FlowBedrockProvider(
            logger, model_name=model_name, max_tokens=max_tokens, temperature=temperature
        )
    else:
