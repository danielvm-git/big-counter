...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test the BCP Calculator with sample user stories."
    )
    parser.add_argument(
        "--executions",
        type=int,
    ...
    for story_file in test_stories:
        ...
        try:
            ...
            for i in range(args.executions):
                ...
                total_bcp_list.append(
                    results.get("steps", {}).get("External Integrations Complexity", "")
                )

            # Save results
            output_file = os.path.join(
                output_path, f"{os.path.splitext(story_file)[0]}_results.json"
            )
    ...
    csv_output_path = os.path.join(
        output_path, f"bcp_results_summary-{datetime.now().strftime('_%Y%m%d_%H%M%S')}.csv"
    )
